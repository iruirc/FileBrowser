...
open class FileBrowser: UINavigationController {
    ...
    /// Available ile types
    open var onlyFileExtensions: [String]? {
        didSet {
            parser.onlyFileExtensions = onlyFileExtensions
        }
    }
    ...
    // Enable directory
    open var enableDirectory: Bool = true {
        didSet {
            parser.enableDirectory = enableDirectory
        }
    }
